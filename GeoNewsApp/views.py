...
def homepage(request):
    if request.method == 'POST':
        ...
        Marker.objects.get_or_create(user=request.user, address=target_address, coordinates = target_coordinates)
        # pdb.set_trace()
        print(Marker.objects.filter(user=request.user.id))
        return render(
            request,
            'homepage.html',
            {
                'bookmarks': Marker.objects.filter(user=request.user.id)
            }
        )
    else:
        return render(request, 'homepage.html', {'cock': "COCK", 'bookmarks': Marker.objects.filter(user=request.user.id)})
